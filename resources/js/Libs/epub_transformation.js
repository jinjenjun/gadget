...
try {
  ...
  if (!inputPath || !outputPath) {
    throw new Error('用法: node epub_transformation.js input.epub output.epub');
  }
  ...
  function findHtmlFiles(dir) {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      } else if (typeof f === 'string' && (f.toLowerCase().endsWith('.xhtml') || f.toLowerCase().endsWith('.html'))) {
        files.push(full);
      }
    }
    return files;
  ...
  if (htmlFiles.length === 0) throw new Error('找不到任何 .xhtml 或 .html 檔案');
  ...
  htmlFiles.forEach((filePath) => {
    ...
    nodes.forEach((node) => {
      ...
      newHtml = newHtml.replace(regex, (_, num) => {
        ...
        return `<img src="../Images/${fallback}" />`;
      });
    ...
    document.querySelectorAll('p.基本段落 span').forEach((span) => {
      span.innerHTML = span.innerHTML.replace(/\[([0-9]+)\]/g, (_, num) => {
        if (!footnotes.includes(num)) footnotes.push(num);
        return `<a href="#note-${num}" id="ref-${num}">(${num})</a>`;
      });
    });
    ...
    document.querySelectorAll('p.基本段落 span').forEach((span) => {
      ...
      if (match) {
        ...
        noteText = noteText.slice(3).replace(/^\(?\d+\)?[\.\s]*|\[\d+\]\s*/g, '');
        span.innerHTML = `<a id="note-${currentFootnoteNumber}" href="#ref-${currentFootnoteNumber}">(${currentFootnoteNumber})</a> ${noteText}`;
        currentFootnoteNumber++;
      }
    });

    const footnoteTitleP = Array.from(document.querySelectorAll('p.基本段落'))
    ...
    if (footnoteTitleP) {
      ...
      while (currentP && currentP.tagName === 'P') {
        if (!currentP.textContent.includes('註釋：') && !currentP.querySelector('a[id^="note-"]')) break;
  ...
  if (fs.existsSync(originalCssDir)) {
    ...
    const originalCssFile = files.find(f => f.toLowerCase().includes('idgeneratedstyles'));
    if (originalCssFile) {
  ...
  const chapters = htmlFiles.map((file, i) => {
    ...
    const chapterTitle = document.querySelector('title') ? document.querySelector('title').textContent : `第${i + 1}章`;
  ...
  fs.writeFileSync(path.join(META_INF, 'container.xml'), `<?xml version="1.0"?>
