...
function generateFakeReadableText(length = 80) {
  const chars = "的一是在不了有人這中大為上個國我以要他時來用們生到作地於出就分對成會可主發年動同工也能下過子說產種面而方後多定行學法所民得經十三之進著等部度家電力裡如水化高自二理起小物現實加量都兩體制機當使點從業本去把性好應開它合還因由其些然前外天政四日那社義事平形相全表間樣與關各重新線內數正心反你明看原又麼利比或但質氣第向道命此變條只沒結解問意建月公無系軍很情者最立代想已通並提直題黨程展五果料象員革位入常文總次品式活設及管特件長求老";
...
function generateOverlayText(length = 80) {
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789你我他她嗎吧呀喔啊★☆※╳■◆";
  const baseText = Array.from({ length }, () => chars[Math.floor(Math.random() * chars.length)]).join("");
  return injectInvisibleChars(baseText);
}
...
async function addInvisibleOverlay(inputPath, outputPath) {
  ...
  if (!fs.existsSync(inputPath)) throw new Error("輸入檔案不存在: " + inputPath);
  ...
  const fontPath = path.join(__dirname, "../../../public/fonts/01金梅毛筆行書.ttf");

  if (!fs.existsSync(fontPath)) throw new Error("找不到字型檔: " + fontPath);
